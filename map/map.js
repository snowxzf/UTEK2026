...
class HospitalDroneTracker {
    constructor() {
        ...
        // Track active flight paths for smooth animation
        this.droneFlights = {};
        ...
        if (this.floorPlanContainer) {
            this.floorPlanContainer.hidden = true;
        }

        if (this.floorSvg) {
            this.floorSvg.setAttribute('viewBox', '0 0 1000 600');
            this.floorSvg.setAttribute('preserveAspectRatio', 'xMidYMid meet');
            this.floorSvg.setAttribute('width', '1000');
            this.floorSvg.setAttribute('height', '600');
        }
    ...
    updateControls() {
        ...
        const makeControl = (label, action, isActive) => {
            const control = document.createElement('div');
            control.className = `tracker-control${isActive ? ' active' : ''}`;
        ...
        this.controlsContainer.appendChild(
            makeControl('Back to 3D View', () => this.switchTo3DView(), this.viewMode === '3d')
        );
        ...
        this.controlsContainer.appendChild(
            makeControl(
                ...
                this.viewMode === '2d' && this.selectedFloor === 1
            )
        );
        ...
        this.controlsContainer.appendChild(
            makeControl(
                ...
                this.viewMode === '2d' && this.selectedFloor === 2
            )
        );
    }
    ...
    switchTo2DView(floor) {
        this.viewMode = '2d';
        this.selectedFloor = floor || this.selectedFloor || 1;
        this.canvasContainer.hidden = true;
        this.floorPlanContainer.hidden = false;
        this.canvasContainer.style.setProperty('display', 'none', 'important');
        this.floorPlanContainer.style.setProperty('display', 'flex', 'important');
        this.canvasContainer.style.visibility = 'hidden';
        this.floorPlanContainer.style.visibility = 'visible';
        this.canvasContainer.style.pointerEvents = 'none';
        this.floorPlanContainer.style.pointerEvents = 'auto';
        this.updateControls();
        this.render2DFloor();
    }
    ...
    switchTo3DView() {
        ...
        this.canvasContainer.hidden = false;
        this.floorPlanContainer.hidden = true;
        this.canvasContainer.style.setProperty('display', 'flex', 'important');
        this.floorPlanContainer.style.setProperty('display', 'none', 'important');
        this.canvasContainer.style.visibility = 'visible';
        this.floorPlanContainer.style.visibility = 'hidden';
        this.canvasContainer.style.pointerEvents = 'auto';
        this.floorPlanContainer.style.pointerEvents = 'none';
        this.droneInfo.style.display = 'none';
        this.updateControls();
        this.onWindowResize();
    ...
    getLocationCoords(locationId) {
        if (window.LOCATION_TO_COORDS && window.LOCATION_TO_COORDS[locationId]) {
            return window.LOCATION_TO_COORDS[locationId];
        }
        return null;
    }

    calculateDronePosition(droneData) {
        const loc = this.getLocationCoords(droneData.current_location_id);
        if (loc) {
    ...
    updateDronePositions() {
        this.drones = this.drones.map((drone) => {
            if (drone.location_id) {
                const loc = this.getLocationCoords(drone.location_id);
                if (loc) {
                    return { ...drone, x: loc.x, y: loc.y };
                }
            }
            return drone;
        });
...
// Initialized by the main dashboard once the DOM is ready.